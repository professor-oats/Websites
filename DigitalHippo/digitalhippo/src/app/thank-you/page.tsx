...
const ThankYouPage = async ({searchParams}: PageProps) => {
  ...
  /* Get the orderId from searchParams */
  ...
  /* Get/await the payload client so we can find the orders in payload */
  ...
  /* Make a check to see that the user is matching the order/eligible for download */

  /* Make a check that we have an userId in valid form (string) */

  const orderUserId = typeof order.user === "string" ? order.user : order.user.id

  /* If orderUserId not equals to the signed in user in the app */
          ...
          <div className="lg:col-start-2">
            ...
            {order._isPaid ? (
              <p className="mt-2 text-base text-muted-foreground">
                Your order was processed and you assets are available
                to download below. We&apos;ve sent you receipt and order
                details to {' '}
              ...
              <p className="mt-2 text-base text-muted-foreground">
                ...
                currently processing it. So hang tight and we&apos;ll send
            ...
            <div className="mt-16 text-sm font-medium">
              ...
              <PaymentStatus isPaid={order._isPaid} orderEmail={(order.user as User).email}
                orderId={order.id}
              >
